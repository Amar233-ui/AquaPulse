...
import type { CitizenDashboardData, CitizenNetworkSector, EahFacility } from "@/lib/types"
...
type ParsedAlert = {
  ...
  type: string
  location: string
  severity: "normal" | "alerte" | "critique"
  time: string
  zone: string
  coordinates: [number, number]
  description: string
  rawSeverity: "normal" | "alerte" | "critique"
}
...
function citizenAnchor(zone: string) {
  const anchor = ZONE_ANCHORS[zone]
  if (!anchor) return anchor
  if (zone !== "Parcelles Assainies") return anchor
  return {
    lat: anchor.lat + PARCELLES_SHIFT.lat,
    lng: anchor.lng + PARCELLES_SHIFT.lng,
  }
}

function citizenNode<T extends { zone?: string; lat: number; lng: number }>(node: T) {
...
const HIDDEN_TOPOLOGY_SUMMARY = `${PIPES.length} conduites masquées · ${INTER_NODES.length} nœuds`

const DAKAR_BOUNDS: [[number, number], [number, number]] = [
  [14.668, -17.485],
  [14.792, -17.250],
]

const DEFAULT_DASHBOARD: CitizenDashboardData = {
  qualityScore: 0,
  temperature: 0,
  networkState: "Normal",
  activeAlerts: 0,
...
const DEFAULT_EAH: EahFeed = { items: [], total: 0 }
const DEFAULT_NETWORK: NetworkFeed = { sectors: [] }

function normalizeText(value: string) {
  return value.normalize("NFD").replace(/[\u0300-\u036f]/g, "").toLowerCase()
}

function inferZoneFromText(value: string, fallback: string) {
  const text = normalizeText(value)
  const found = QUARTIERS.find((zone) => text.includes(normalizeText(zone)))
  return found ?? fallback
}

function offsetPoint(base: { lat: number; lng: number }, index: number, total: number, radius = 0.0024): [number, number] {
  if (total <= 1) return [base.lat, base.lng]
  const angle = (index / total) * Math.PI * 2
  const stretch = 1 + (index % 3) * 0.12
  return [base.lat + Math.cos(angle) * radius * stretch, base.lng + Math.sin(angle) * radius * 1.25 * stretch]
}
...
function pipeStyle(pipe: { diameter_mm?: number; risk?: string }) {
  const diameter = pipe.diameter_mm || 150
  const width = Math.max(1.1, diameter / 180)
  const opacity = pipe.risk === "high" ? 0.24 : pipe.risk === "medium" ? 0.2 : 0.16
  return { width, opacity }
}

function severityTone(severity: "normal" | "alerte" | "critique") {
  if (severity === "critique") return "#f87171"
  if (severity === "alerte") return "#fbbf24"
  return "#22d3ee"
}

function facilityEmoji(type: EahFacility["type"]) {
  if (type === "latrine_publique") return "🚻"
  if (type === "station_lavage_mains") return "🖐"
  if (type === "bloc_hygiene") return "🧼"
  return "🚰"
}

function confirmationLabel(value?: EahFacility["community_confirmation"]) {
  if (value === "confirmed") return "Confirmé communauté"
  if (value === "probable") return "Probable"
  if (value === "to_verify") return "À vérifier"
  return "Aucun signalement"
}

function confirmationColor(value?: EahFacility["community_confirmation"]) {
  if (value === "confirmed") return "#f472b6"
  if (value === "probable") return "#22d3ee"
  if (value === "to_verify") return "#fbbf24"
  return "#64748b"
}

function zoneTone(input: {
  sector?: CitizenNetworkSector
  alerts: ParsedAlert[]
  facilities: EahFacility[]
}) {
  const criticalAlerts = input.alerts.filter((alert) => alert.severity === "critique").length
  const alertAlerts = input.alerts.filter((alert) => alert.severity === "alerte").length
  const degradedEah = input.facilities.filter((facility) => facility.status === "degradé").length
  const outEah = input.facilities.filter((facility) => facility.status === "hors_service").length

  if (criticalAlerts > 0 || input.sector?.status === "critique" || outEah > 0) {
    return { potable: false, message: "Incident", color: "#f87171" }
  }

  if (alertAlerts > 0 || input.sector?.status === "alerte" || degradedEah > 0) {
    return { potable: false, message: "Surveillance", color: "#fbbf24" }
  }

  if (input.sector) {
    return { potable: input.sector.status === "normal", message: "Eau conforme", color: "#34d399" }
  }

  return { potable: true, message: "Eau conforme", color: "#34d399" }
}
...
export function DakarCitizenMap() {
  ...
  const dashboardUrl = `/api/citoyen/dashboard?quartier=${encodeURIComponent(quartier)}`
  const { data: dashboard, loading: dashboardLoading, error: dashboardError } = useApiQuery<CitizenDashboardData>(dashboardUrl, DEFAULT_DASHBOARD)
  const { data: eahFeed, loading: eahLoading, error: eahError } = useApiQuery<EahFeed>("/api/citoyen/eah", DEFAULT_EAH)
  const { data: networkFeed, loading: networkLoading, error: networkError } = useApiQuery<NetworkFeed>("/api/citoyen/reseau", DEFAULT_NETWORK)

  useEffect(() => {
    try {
      const saved = window.localStorage.getItem(STORAGE_KEY)
      if (saved && QUARTIERS.includes(saved)) {
        setQuartier(saved)
      }
    } catch {}
  }, [])
  ...
  useEffect(() => {
    try {
      window.localStorage.setItem(STORAGE_KEY, quartier)
    } catch {}
  }, [quartier])
  ...
  useEffect(() => {
    const timer = window.setInterval(() => setClock(new Date().toLocaleTimeString("fr-FR")), 1000)
    ...
    if (mapRef.current) {
      window.setTimeout(() => {
        ...
        if (leafletRef.current) {
          const bounds = leafletRef.current.latLngBounds(DAKAR_BOUNDS)
          const fitZoom = mapRef.current.getBoundsZoom(bounds, false)
          mapRef.current.setMinZoom(Math.max(10, fitZoom))
          mapRef.current.panInsideBounds(DAKAR_BOUNDS, { animate: false })
        }
      }, 300)
    }
  }, [open])
  ...
  useEffect(() => {
    ...
    import("leaflet").then((lm) => {
      const Lf = lm.default ?? lm
      if (mapRef.current || !mapDiv.current) return

      const map = Lf.map(mapDiv.current, {
        center: [14.725, -17.4],
        zoom: 12,
        maxZoom: 15,
        maxBounds: DAKAR_BOUNDS,
      ...
      const dakarBounds = Lf.latLngBounds(DAKAR_BOUNDS)
      const fitZoom = map.getBoundsZoom(dakarBounds, false)
      map.setMinZoom(Math.max(10, fitZoom))
      map.fitBounds(dakarBounds.pad(0.02), { animate: false })
  ...
  const parsedAlerts = useMemo<ParsedAlert[]>(() => {
    if (dashboardError) {
      return ALERTS.map((alert: any) => ({
        id: alert.alert_id,
        type: alert.type,
        location: alert.location,
        severity: alert.severity === "Critique" ? "critique" : alert.severity === "Alerte" ? "alerte" : "normal",
        time: alert.date,
        zone: inferZoneFromText(alert.location, quartier),
        coordinates: [alert.lat, alert.lng],
        description: alert.description,
        rawSeverity: alert.severity === "Critique" ? "critique" : alert.severity === "Alerte" ? "alerte" : "normal",
      }))
    }

    return dashboard.recentAlerts.map((alert) => {
      const [incidentType, locationFromMessage] = alert.message.split(" - ")
      const zone = inferZoneFromText(locationFromMessage ?? alert.message, quartier)
      const anchor = citizenAnchor(zone) ?? citizenAnchor(quartier) ?? ZONE_ANCHORS[zone] ?? ZONE_ANCHORS[quartier]
      const grouped = dashboard.recentAlerts.filter((item) => inferZoneFromText(item.message.split(" - ")[1] ?? item.message, quartier) === zone)
      const index = grouped.findIndex((item) => item.id === alert.id)
      const [lat, lng] = offsetPoint(anchor, Math.max(index, 0), Math.max(grouped.length, 1))

      return {
        id: alert.id,
        type: incidentType || "Alerte",
        location: locationFromMessage ?? quartier,
        severity: alert.type,
        time: alert.time,
        zone,
        coordinates: [lat, lng],
        description: `Signalement réseau détecté dans ${locationFromMessage ?? zone}.`,
        rawSeverity: alert.type,
      }
    })
  }, [dashboard.recentAlerts, dashboardError, quartier])

  const sectorMap = useMemo(() => {
    return new Map(networkFeed.sectors.map((sector) => [sector.name, sector]))
  }, [networkFeed.sectors])

  const alertsByZone = useMemo(() => {
    const grouped = new Map<string, ParsedAlert[]>()
    QUARTIERS.forEach((zone) => grouped.set(zone, []))
    parsedAlerts.forEach((alert) => {
      const list = grouped.get(alert.zone) ?? []
      list.push(alert)
      grouped.set(alert.zone, list)
    })
    return grouped
  }, [parsedAlerts])

  const facilitiesByZone = useMemo(() => {
    const grouped = new Map<string, EahFacility[]>()
    QUARTIERS.forEach((zone) => grouped.set(zone, []))
    eahFeed.items.forEach((facility) => {
      const list = grouped.get(facility.quartier) ?? []
      list.push(facility)
      grouped.set(facility.quartier, list)
    })
    return grouped
  }, [eahFeed.items])

  const zoneCards = useMemo(() => {
    if (dashboardError || networkError || eahError) {
      return QUARTIERS.map((zone) => {
        const legacy = ZONE_STATUS[zone] ?? { potable: true, message: "Eau conforme", color: "#34d399" }
        const zoneAlerts = ALERTS.filter((alert: any) => inferZoneFromText(alert.location, zone) === zone)
        return {
          quartier: zone,
          status: legacy.message,
          color: legacy.color,
          potable: legacy.potable,
          pressure: "-",
          health: "-",
          sensors: "-",
    ...
    return QUARTIERS.map((zone) => {
      const sector = sectorMap.get(zone)
      const zoneAlerts = alertsByZone.get(zone) ?? []
      const zoneFacilities = facilitiesByZone.get(zone) ?? []
      const status = zoneTone({ sector, alerts: zoneAlerts, facilities: zoneFacilities })
      const operational = zoneFacilities.filter((facility) => facility.status === "operationnel").length
      const issues = zoneFacilities.filter((facility) => facility.status !== "operationnel").length

      return {
        quartier: zone,
        status: status.message,
        color: status.color,
        potable: status.potable,
        pressure: sector?.pressure ? `${sector.pressure} bar` : "-",
        health: sector?.health ?? 0,
        sensors: sector?.sensors ?? 0,
        alerts: zoneAlerts.length,
        eahTotal: zoneFacilities.length,
        eahOperational: operational,
        eahIssues: issues,
        eahColor: issues > 0 ? "#fbbf24" : "#34d399",
      }
    })
  }, [alertsByZone, dashboardError, eahError, facilitiesByZone, networkError, sectorMap])

  const selectedAlert = useMemo(() => parsedAlerts.find((alert) => alert.id === selectedAlertId) ?? null, [parsedAlerts, selectedAlertId])
  const selectedFacility = useMemo(() => eahFeed.items.find((facility) => facility.id === selectedFacilityId) ?? null, [eahFeed.items, selectedFacilityId])
  const selectedZone = useMemo(() => zoneCards.find((zone) => zone.quartier === quartier) ?? zoneCards[0], [quartier, zoneCards])
  const selectedZoneAlerts = alertsByZone.get(quartier) ?? []
  const selectedZoneFacilities = facilitiesByZone.get(quartier) ?? []
  const currentSector = sectorMap.get(quartier)

  useEffect(() => {
    if (!ready || !mapRef.current) return
    const anchor = citizenAnchor(quartier)
    if (anchor) {
      mapRef.current.setView([anchor.lat, anchor.lng], 12.8)
      mapRef.current.panInsideBounds(DAKAR_BOUNDS, { animate: false })
    }
  }, [quartier, ready])
  ...
  useEffect(() => {
    ...
    const overlay = Lf.layerGroup().addTo(map)
    overlayRef.current = overlay

    const nodeMap: Record<string, any> = {}
    NODES.map((node: any) => citizenNode(node)).forEach((node: any) => {
      nodeMap[node.id] = node
    })
    INTER_NODES.map((node: any) => citizenNode(node)).forEach((node: any) => {
      nodeMap[node.id] = node
    })

    // Vue citoyenne épurée: on masque la topologie de conduites,
    // utile surtout pour l'opérateur, pas pour la lecture publique.
    PIPES.forEach((pipe: any) => {
      const from = nodeMap[pipe.from]
      const to = nodeMap[pipe.to]
      if (!from || !to) return
      const style = pipeStyle(pipe)
      const waypoints = routeWaypoints(from, to, pipe.id)

      Lf.polyline(waypoints, {
        color: "#0ea5e9",
        weight: style.width + 1.8,
        opacity: 0.08,
        lineCap: "round",
        lineJoin: "round",
      }).addTo(overlay)
      Lf.polyline(
        waypoints,
        {
          ...
          lineCap: "round",
          lineJoin: "round",
          className: "aqp-flow-link",
        },
      ).addTo(overlay)
    })

    const zoneNodeGroups = QUARTIERS.map((zone) => ({
      zone,
      nodes: INTER_NODES.map((node: any) => citizenNode(node)).filter((node: any) => node.zone === zone).sort((a: any, b: any) => {
        const anchor = citizenAnchor(zone) ?? ZONE_ANCHORS[zone]
    ...
    zoneNodeGroups.forEach(({ zone, nodes }) => {
      const anchor = citizenAnchor(zone)
      if (!anchor || nodes.length === 0) return

      nodes.forEach((node: any, index: number) => {
        const anchorNode = citizenNode({ lat: anchor.lat, lng: anchor.lng, zone })
        Lf.polyline(
          routeWaypoints(anchorNode, node, `${zone}:${node.id}:anchor`),
          {
            color: "#5ecbff",
            weight: 0.9,
            opacity: 0.14,
        ...
        if (next) {
          Lf.polyline(
            routeWaypoints(node, next, `${zone}:${node.id}:${next.id}`),
            {
              color: "#5ecbff",
              weight: 0.8,
              opacity: 0.13,
        ...
        if (skip && index % 2 === 0) {
          Lf.polyline(
            routeWaypoints(node, skip, `${zone}:${node.id}:${skip.id}`),
            {
              color: "#5ecbff",
              weight: 0.7,
              opacity: 0.08,
    ...
    zoneCards.forEach((zone) => {
      const anchor = citizenAnchor(zone.quartier)
      if (!anchor) return
      const selected = quartier === zone.quartier
      const border = selected ? zone.color : `${zone.color}AA`
      const fill = selected ? zone.color : `${zone.color}12`
      const outerRadiusM = selected ? 920 : 640
      ...
      Lf.circle([anchor.lat, anchor.lng], {
        ...
        weight: selected ? 2 : 1,
        fillColor: fill,
        fillOpacity: selected ? 0.13 : 0.06,
        opacity: selected ? 1 : 0.65,
      })
        .addTo(overlay)
        .on("click", () => {
          setQuartier(zone.quartier)
      ...
      Lf.marker([anchor.lat, anchor.lng], {
        icon: Lf.divIcon({
          html: `<div style="background:rgba(2,6,23,.78);border:1px solid ${border};color:#f3f4f6;padding:4px 8px;border-radius:999px;font-size:10px;font-weight:700;box-shadow:0 0 10px rgba(0,0,0,.22);white-space:nowrap">${zone.quartier}</div>`,
          className: "",
          iconSize: [100, 24],
          iconAnchor: [45, 12],
        }),
    ...
    const greyLinks: Array<[string, string]> = [
      ["Plateau", "Médina"],
      ["Médina", "Fann"],
      ["Fann", "HLM"],
      ["HLM", "Grand Dakar"],
      ["Grand Dakar", "Parcelles Assainies"],
      ["Parcelles Assainies", "Pikine"],
      ["Pikine", "Guédiawaye"],
      ["Pikine", "Pikine"],
    ]

    greyLinks.forEach(([fromZone, toZone]) => {
      const from = citizenAnchor(fromZone)
      const to = citizenAnchor(toZone)
      if (!from || !to) return

      Lf.polyline(
        routeWaypoints(from, to, `${fromZone}:${toZone}`),
        {
          color: "#5ecbff",
          weight: 1,
          opacity: 0.2,
    ...
    NODES.filter((node: any) => node.type === "reservoir").forEach((node: any) => {
      const adjusted = citizenNode(node)
      Lf.marker([adjusted.lat, adjusted.lng], {
        icon: Lf.divIcon({
          html: `<div style="width:26px;height:26px;background:#38bdf818;border:2px solid #38bdf8;border-radius:6px;display:flex;align-items:center;justify-content:center;font-size:13px">💧</div>`,
          className: "",
          iconSize: [26, 26],
          iconAnchor: [13, 13],
        }),
        ...
        .addTo(overlay)
        .bindTooltip(
          `<div style="background:#0f172a;border:1px solid #38bdf833;color:#e2e8f0;padding:5px 9px;border-radius:6px;font-size:12px">${adjusted.name}</div>`,
    ...
    zoneCards.forEach((zone) => {
      ...
      if (!anchor) return
      const selected = quartier === zone.quartier
      Lf.circleMarker([anchor.lat, anchor.lng], {
        ...
        .bindTooltip(
          `<div style="background:#0f172a;border:1px solid ${zone.color}33;color:#e2e8f0;padding:6px 10px;border-radius:8px;font-size:12px">
            <div style="font-weight:700;margin-bottom:2px">${zone.quartier}</div>
            <div style="color:${zone.color}">${zone.status}</div>
          </div>`,
          { sticky: true, opacity: 1 },
        )
    })
    ...
    parsedAlerts.forEach((alert, index) => {
      const color = severityTone(alert.severity)
      const [lat, lng] = alert.coordinates
      const isSelected = selectedAlertId === alert.id
      Lf.circle([lat, lng], {
        radius: alert.severity === "critique" ? 145 : 110,
        color,
        fillColor: color,
        fillOpacity: 0.035,
        weight: 1,
      ...
      Lf.marker([lat, lng], {
        icon: Lf.divIcon({
          html: `<div style="width:${isSelected ? 30 : 26}px;height:${isSelected ? 30 : 26}px;border-radius:50%;border:2px solid ${color};background:rgba(15,23,42,.88);box-shadow:0 0 8px ${color}44;display:flex;align-items:center;justify-content:center;font-size:12px;color:${color}">⚠</div>`,
          className: "",
          iconSize: [32, 32],
          iconAnchor: [16, 16],
        ...
        .on("click", () => {
          ...
          setSelectedFacilityId(null)
          setQuartier(alert.zone)
          mapRef.current?.panInsideBounds(DAKAR_BOUNDS, { animate: false })
          if (mob) setDrw(true)
        })
    })

    eahFeed.items.forEach((facility, index) => {
      const anchor = citizenAnchor(facility.quartier) ?? citizenAnchor(quartier) ?? ZONE_ANCHORS[facility.quartier] ?? ZONE_ANCHORS[quartier]
      const zoneFacilities = facilitiesByZone.get(facility.quartier) ?? []
      const facilityIndex = zoneFacilities.findIndex((item) => item.id === facility.id)
      const [lat, lng] = offsetPoint(anchor, Math.max(facilityIndex, index), Math.max(zoneFacilities.length, 1), 0.0019)
      const color = facility.status === "hors_service" ? "#f87171" : facility.status === "degradé" ? "#fbbf24" : "#34d399"
      const selected = selectedFacilityId === facility.id

      Lf.marker([lat, lng], {
        icon: Lf.divIcon({
          html: `<div style="width:${selected ? 28 : 24}px;height:${selected ? 28 : 24}px;border-radius:50%;border:2px solid ${color};background:rgba(15,23,42,.9);box-shadow:0 0 8px ${color}35;display:flex;align-items:center;justify-content:center;font-size:12px">${facilityEmoji(facility.type)}</div>`,
          className: "",
          iconSize: [28, 28],
          iconAnchor: [14, 14],
        ...
        .addTo(overlay)
        .bindTooltip(
          `<div style="background:#0f172a;border:1px solid ${color}33;color:#e2e8f0;padding:6px 10px;border-radius:8px;font-size:12px">
            <div style="font-weight:700;margin-bottom:2px">${facility.name}</div>
            <div style="color:${color}">${facility.status}</div>
          </div>`,
          { sticky: true, opacity: 1 },
        )
        .on("click", () => {
  ...
  }, [ready, zoneCards, parsedAlerts, eahFeed.items, facilitiesByZone, selectedAlertId, selectedFacilityId, quartier, mob])

  const dashboardLoadingState = dashboardLoading || networkLoading || eahLoading
  const useLegacyFallback = Boolean(dashboardError)
  const hasLiveIssue = Boolean(dashboardError || networkError || eahError)
  ...
  const SB = () => (
    <div style={{ display: "flex", flexDirection: "column", gap: 14, padding: 14, fontFamily: "monospace" }}>
      <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
        ...
        <div style={{ color: hasLiveIssue ? "#fbbf24" : "#94a3b8", fontSize: 10, lineHeight: 1.5 }}>
          {hasLiveIssue ? "Mode secours historique" : "Données live du quartier et du réseau"}
        </div>
        <div style={{ color: "#64748b", fontSize: 10, lineHeight: 1.4 }}>{HIDDEN_TOPOLOGY_SUMMARY}</div>
      </div>

      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8 }}>
        <div style={{ border: "1px solid rgba(34,211,238,.14)", borderRadius: 8, padding: "8px 10px", background: "#0f172a" }}>
          <div style={{ color: "#64748b", fontSize: 9, letterSpacing: "0.12em", marginBottom: 2 }}>QUALITÉ</div>
      ...
      <div>
        <p style={{ color: "#22d3ee", fontSize: 9, fontWeight: 700, letterSpacing: "0.15em", marginBottom: 7 }}>MON QUARTIER</p>
        {zoneCards.map((zone) => (
          <div
            key={zone.quartier}
            onClick={() => {
            ...
            style={{
              ...
              cursor: "pointer",
              marginBottom: 3,
              background: quartier === zone.quartier ? `${zone.color}18` : "transparent",
              border: `1px solid ${quartier === zone.quartier ? `${zone.color}44` : "transparent"}`,
              transition: "all .15s",
            }}
          >
            <div style={{ display: "flex", alignItems: "center", gap: 7 }}>
              <div style={{ width: 7, height: 7, borderRadius: "50%", background: zone.color }} />
              <span style={{ color: "#94a3b8", fontSize: 11 }}>{zone.quartier}</span>
            </div>
            <span style={{ color: zone.color, fontSize: 10, fontWeight: 700 }}>{zone.status}</span>
          </div>
        ))}
      </div>

      <div>
        <p style={{ color: "#f87171", fontSize: 9, fontWeight: 700, letterSpacing: "0.15em", marginBottom: 7 }}>
          INCIDENTS&nbsp;
          <span style={{ background: "#f8717133", borderRadius: 3, padding: "1px 5px" }}>{parsedAlerts.length}</span>
        </p>
        {(useLegacyFallback ? ALERTS : parsedAlerts).map((alert: any) => {
          const isLegacy = useLegacyFallback
          const color = isLegacy ? SEV[alert.severity] : severityTone(alert.severity)
          return (
            <div
              key={isLegacy ? alert.alert_id : alert.id}
              onClick={() => {
                if (isLegacy) {
                  setSelectedAlertId(alert.alert_id)
                  setSelectedFacilityId(null)
                  setQuartier(inferZoneFromText(alert.location, quartier))
                  mapRef.current?.setView([alert.lat, alert.lng], 14)
                } else {
                  setSelectedAlertId(alert.id)
                  setSelectedFacilityId(null)
                  setQuartier(alert.zone)
                  mapRef.current?.setView([alert.coordinates[0], alert.coordinates[1]], 14)
                }
                if (mob) setDrw(true)
              }}
              style={{
                ...
                cursor: "pointer",
                marginBottom: 4,
                background: (isLegacy ? selectedAlertId === alert.alert_id : selectedAlertId === alert.id) ? `${color}10` : "transparent",
            ...
            >
              <div style={{ color, fontSize: 11, fontWeight: 700, marginBottom: 1 }}>{isLegacy ? alert.type : alert.type}</div>
              <div style={{ color: "#64748b", fontSize: 10 }}>{isLegacy ? alert.location : `${alert.location} · ${alert.zone}`}</div>
            </div>
          )
        })}
      </div>
          ...
          return (
            <div
              ...
              style={{
                ...
                cursor: "pointer",
                marginBottom: 4,
                background: selectedFacilityId === facility.id ? `${color}10` : "transparent",
                borderLeft: `3px solid ${color}`,
                borderTop: `1px solid ${color}18`,
                borderRight: `1px solid ${color}18`,
                borderBottom: `1px solid ${color}18`,
            ...
            >
              <div style={{ color: "#e2e8f0", fontSize: 11, fontWeight: 700, marginBottom: 1 }}>{facility.name}</div>
              <div style={{ color, fontSize: 10 }}>{confirmationLabel(facility.community_confirmation)}</div>
            </div>
          )
        })}
      </div>
  ...
  const DC = () => {
    if (selectedAlert) {
      const color = severityTone(selectedAlert.severity)
      return (
        <div style={{ padding: 14, display: "flex", flexDirection: "column", gap: 9 }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
            <span style={{ color: "#22d3ee", fontSize: 9, fontWeight: 700, letterSpacing: "0.15em" }}>INCIDENT</span>
            <button onClick={() => { setSelectedAlertId(null); if (mob) setDrw(false) }} style={{ color: "#475569", background: "none", border: "none", cursor: "pointer", fontSize: 16 }}>✕</button>
          </div>
          <div style={{ background: `${color}0d`, border: `1px solid ${color}33`, borderRadius: 8, padding: "10px 12px" }}>
            <div style={{ color: "#f3f4f6", fontSize: 14, fontWeight: 800, marginBottom: 3 }}>{selectedAlert.severity === "critique" ? "🔴" : "⚠️"} {selectedAlert.type}</div>
            <div style={{ color: "#e5e7eb", fontSize: 11 }}>{selectedAlert.location}</div>
          </div>
          <p style={{ color: "#d1d5db", fontSize: 12, lineHeight: 1.65, margin: 0 }}>{selectedAlert.description}</p>
          <div style={{ display: "flex", justifyContent: "space-between", fontSize: 11 }}><span style={{ color: "#cbd5e1" }}>Statut</span><span style={{ color: "#f3f4f6", fontWeight: 700 }}>{selectedAlert.rawSeverity === "critique" ? "Critique" : selectedAlert.rawSeverity === "alerte" ? "Alerte" : "Normal"}</span></div>
          <div style={{ background: "#0f172a", border: "1px solid rgba(34,211,238,.12)", borderRadius: 8, padding: "8px 10px" }}>
            <div style={{ color: "#cbd5e1", fontSize: 10, marginBottom: 2 }}>Quartier détecté</div>
            <div style={{ color: "#f3f4f6", fontSize: 12, fontWeight: 700 }}>{selectedAlert.zone}</div>
          </div>
          {selectedAlert.rawSeverity === "critique" && (
            <div style={{ background: "#fbbf2411", border: "1px solid #fbbf2433", borderRadius: 6, padding: "8px 10px" }}>
              <p style={{ color: "#fbbf24", fontSize: 11, margin: 0 }}>💡 Conservez de l'eau en bouteille par précaution.</p>
            </div>
          )}
          <button
            onClick={() => { setQuartier(selectedAlert.zone); setSelectedAlertId(null); if (mob) setDrw(false) }}
            style={{ background: "#22d3ee", color: "#082f49", border: "none", borderRadius: 8, padding: "9px 12px", fontWeight: 700, cursor: "pointer" }}
          >
            Voir le quartier
          </button>
        </div>
    ...
    if (selectedFacility) {
      const color = confirmationColor(selectedFacility.community_confirmation)
      return (
        <div style={{ padding: 14, display: "flex", flexDirection: "column", gap: 9 }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
          ...
          <div style={{ background: `${color}0d`, border: `1px solid ${color}33`, borderRadius: 8, padding: "10px 12px" }}>
            <div style={{ color: "#f3f4f6", fontSize: 14, fontWeight: 800, marginBottom: 3 }}>{facilityEmoji(selectedFacility.type)} {selectedFacility.name}</div>
            <div style={{ color: "#e5e7eb", fontSize: 11 }}>{selectedFacility.quartier}</div>
          </div>
          <div style={{ display: "flex", justifyContent: "space-between", fontSize: 11 }}>
            <span style={{ color: "#cbd5e1" }}>Statut</span>
            <span style={{ color: "#f3f4f6", fontWeight: 700 }}>{selectedFacility.status}</span>
          </div>
          <div style={{ display: "flex", justifyContent: "space-between", fontSize: 11 }}>
            <span style={{ color: "#cbd5e1" }}>Signalements</span>
            <span style={{ color: "#f3f4f6", fontWeight: 700 }}>{selectedFacility.community_signal_count ?? 0}</span>
          </div>
          <div style={{ display: "flex", justifyContent: "space-between", fontSize: 11 }}>
            <span style={{ color: "#cbd5e1" }}>Confirmation</span>
            <span style={{ color: "#f3f4f6", fontWeight: 700 }}>{confirmationLabel(selectedFacility.community_confirmation)}</span>
          </div>
          {selectedFacility.notes && (
            <p style={{ color: "#d1d5db", fontSize: 12, lineHeight: 1.6, margin: 0 }}>{selectedFacility.notes}</p>
          )}
          <button
            onClick={() => { setQuartier(selectedFacility.quartier); setSelectedFacilityId(null); if (mob) setDrw(false) }}
            style={{ background: "#22d3ee", color: "#082f49", border: "none", borderRadius: 8, padding: "9px 12px", fontWeight: 700, cursor: "pointer" }}
          >
            Centrer ce quartier
          </button>
    ...
    const zoneColor = selectedZone?.color ?? "#34d399"
    return (
      <div style={{ padding: 14, display: "flex", flexDirection: "column", gap: 9 }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
          <span style={{ color: "#22d3ee", fontSize: 9, fontWeight: 700, letterSpacing: "0.15em" }}>MON QUARTIER</span>
          <button onClick={() => { if (mob) setDrw(false) }} style={{ color: "#475569", background: "none", border: "none", cursor: "pointer", fontSize: 16 }}>✕</button>
        </div>
        <div style={{ background: `${zoneColor}12`, border: `1px solid ${zoneColor}44`, borderRadius: 8, padding: "12px 14px", textAlign: "center" }}>
          <div style={{ fontSize: 22, marginBottom: 3 }}>{selectedZone?.potable ? "✅" : "⚠️"}</div>
          <div style={{ color: "#f3f4f6", fontSize: 14, fontWeight: 800 }}>{selectedZone?.status ?? "Eau conforme"}</div>
          <div style={{ color: "#e5e7eb", fontSize: 11, marginTop: 3 }}>{quartier}</div>
        </div>
        <p style={{ color: "#d1d5db", fontSize: 12, lineHeight: 1.6, margin: 0 }}>
          {selectedZone?.potable
            ? "L'eau est conforme dans votre quartier."
            : "Une anomalie a été détectée. Les équipes et les signaux terrain sont en suivi."}
        </p>
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8 }}>
          <div style={{ background: "#0f172a", border: "1px solid rgba(34,211,238,.12)", borderRadius: 8, padding: "8px 10px" }}>
            <div style={{ color: "#cbd5e1", fontSize: 9, letterSpacing: "0.12em", marginBottom: 2 }}>CAPTEURS</div>
            <div style={{ color: "#f3f4f6", fontSize: 14, fontWeight: 700 }}>{dashboard.activeSensorsRate}%</div>
          </div>
          <div style={{ background: "#0f172a", border: "1px solid rgba(34,211,238,.12)", borderRadius: 8, padding: "8px 10px" }}>
            <div style={{ color: "#cbd5e1", fontSize: 9, letterSpacing: "0.12em", marginBottom: 2 }}>PRESSION</div>
            <div style={{ color: "#f3f4f6", fontSize: 14, fontWeight: 700 }}>{currentSector?.pressure ?? `${dashboard.pressureRate}%`}</div>
          </div>
        </div>
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8 }}>
          <div style={{ background: "#0f172a", border: "1px solid rgba(34,211,238,.12)", borderRadius: 8, padding: "8px 10px" }}>
            <div style={{ color: "#cbd5e1", fontSize: 9, letterSpacing: "0.12em", marginBottom: 2 }}>ALERTES</div>
        ...
        <div style={{ display: "grid", gap: 6 }}>
          {dashboard.waterQualityIndicators.map((indicator) => (
            <div key={indicator.label} style={{ display: "flex", justifyContent: "space-between", fontSize: 11, padding: "6px 8px", borderRadius: 6, background: "#0f172a", border: "1px solid rgba(34,211,238,.08)" }}>
              <span style={{ color: "#cbd5e1" }}>{indicator.label}</span>
              <span style={{ color: "#f3f4f6", fontWeight: 600 }}>{indicator.value}</span>
            </div>
          ))}
        </div>
        {selectedZoneAlerts.length > 0 && (
          <div>
            <p style={{ color: "#cbd5e1", fontSize: 10, marginBottom: 4 }}>Incidents en cours :</p>
            {selectedZoneAlerts.slice(0, 4).map((alert) => (
              <div key={alert.id} style={{ padding: "5px 8px", borderRadius: 5, background: `${severityTone(alert.severity)}0d`, borderLeft: `3px solid ${severityTone(alert.severity)}`, marginBottom: 3, color: "#e5e7eb", fontSize: 11 }}>
        ...
        <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
          <a href={`/citoyen/signaler?quartier=${encodeURIComponent(quartier)}${selectedZoneFacilities[0] ? `&mode=eah&eah=${selectedZoneFacilities[0].id}` : ""}`} style={{ background: "#22d3ee", color: "#082f49", border: "none", borderRadius: 8, padding: "9px 12px", fontWeight: 700, textDecoration: "none", display: "inline-flex", alignItems: "center", justifyContent: "center" }}>
            Signaler ici
          </a>
  ...
  return (
    <div style={{ fontFamily: "monospace", position: "relative", width: "100%", height: "100%", display: "flex" }}>
      <style>{`
        .czb::-webkit-scrollbar{width:3px}
        .czb::-webkit-scrollbar-thumb{background:rgba(34,211,238,.25);border-radius:2px}
      ...
      `}</style>
      {!mob && (
        <div style={{ width: open ? W : 0, minWidth: open ? W : 0, height: "100%", overflow: "hidden", transition: "width .25s,min-width .25s", background: "#020817", borderRight: "1px solid rgba(34,211,238,.22)", flexShrink: 0 }}>
          <div className="czb" style={{ width: W, height: "100%", overflowY: "auto", opacity: open ? 1 : 0, transition: "opacity .2s" }}>
            <SB />
          </div>
        </div>
      )}
      <div style={{ flex: 1, position: "relative", minWidth: 0 }}>
        {!mob && (
          <button onClick={() => setOpen((current) => !current)} style={{ position: "absolute", left: 0, top: "50%", transform: "translateY(-50%)", zIndex: 1000, width: 22, height: 54, background: "#020817", border: "1px solid rgba(34,211,238,.35)", borderLeft: "none", borderRadius: "0 8px 8px 0", cursor: "pointer", color: "#22d3ee", fontSize: 12, display: "flex", alignItems: "center", justifyContent: "center", boxShadow: "3px 0 12px rgba(0,0,0,.5)" }}>{open ? "‹" : "›"}</button>
        )}
        {mob && !drw && (
          <button onClick={() => setDrw(true)} style={{ position: "absolute", bottom: 88, left: 12, zIndex: 1000, background: "#020817", border: "1px solid rgba(34,211,238,.4)", borderRadius: 12, padding: "9px 14px", color: "#22d3ee", fontSize: 11, fontWeight: 700, cursor: "pointer" }}>💧 Mon eau</button>
        )}
        ...
        <div style={{ position: "absolute", bottom: 40, right: 12, zIndex: 1000, background: "#020817", border: "1px solid rgba(34,211,238,.22)", borderRadius: 10, padding: "10px 14px" }}>
          ...
          {[{ c: "#34d399", l: "Eau conforme" }, { c: "#fbbf24", l: "Surveillance" }, { c: "#f87171", l: "Incident" }, { c: "#22d3ee", l: "Réseau" }].map((x) => (
        ...
        {!mob && (selectedAlert || selectedFacility || selectedZone) && (
          <div style={{ position: "absolute", right: 12, top: 12, width: 260, zIndex: 1000, background: "#020817", border: "1px solid rgba(34,211,238,.22)", borderRadius: 10, maxHeight: "calc(100% - 24px)", overflowY: "auto" }}>
            <DC />
          </div>
        )}
        {dashboardLoadingState && (
          <div style={{ position: "absolute", left: 12, top: 12, zIndex: 1000, background: "#020817", border: "1px solid rgba(34,211,238,.18)", borderRadius: 10, padding: "8px 10px", color: "#94a3b8", fontSize: 10 }}>
      ...
      {mob && (
        <>
          ...
          <div style={{ position: "absolute", bottom: 0, left: 0, right: 0, zIndex: 30, background: "#020817", borderRadius: "18px 18px 0 0", border: "1px solid rgba(34,211,238,.22)", borderBottom: "none", boxShadow: "0 -8px 40px rgba(0,0,0,.6)", transform: drw ? "translateY(0)" : "translateY(100%)", transition: "transform .3s cubic-bezier(.32,.72,0,1)", maxHeight: "72vh", display: "flex", flexDirection: "column" }}>
            ...
            <div className="czb" style={{ flex: 1, overflowY: "auto" }}>{(selectedAlert || selectedFacility || selectedZone) ? <DC /> : <SB />}</div>
          </div>
        </>
      )}
    </div>
